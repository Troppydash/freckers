...
public:
                              ...
                              const move &prev_move, int starting_index)
    {
        ...
        std::swap(moves[best_move_index], moves[starting_index]);
    }
    ...
    void incr_history(move &move, int depth)
    {
        ...
        if (m_history[m_pos.m_turn][coord.first][coord.second] >= param::base_score - 100)
        {
            for (auto &i: m_history)
            {
                for (auto &j: i)
    ...
    //     auto coord = move.get_coords();
    //     m_history[m_pos.m_turn][coord.first][coord.second] +=
    //             clampedBonus - m_history[m_pos.m_turn][coord.first][coord.second] * abs(clampedBonus) / MAX_HISTORY;
    // }
    ...
    void decr_history(move &move, int depth)
    {
        ...
        score -= depth * depth;
        if (score < -(param::base_score - 100))
            score = -(param::base_score - 100);
    }
    ...
    int negamax(int depth, int ply, int alpha, int beta, const move &prev_move, bool do_null = true)
    {
        ...
        if (depth >= 5 && depth <= 7 && !is_pv_node && !m_pos.has_jumps())
        {
            // 95%
            const float t = 2;
            const float sigma = 200.0;
            const float a = 1.1;
            const float b = -4.0;

            // if shallow >= beta
            int bound = static_cast<int>(std::round((t * sigma + beta - b) / a));
            if (negamax((depth + 1) / 2, ply + 1, bound - 1, bound, prev_move, true) >= bound)
            {
                return beta;
            }

            // if shallow <= alpha
            bound = static_cast<int>(std::round((-t * sigma + alpha - b) / a));
            if (negamax((depth + 1) / 2, ply + 1, bound, bound + 1, prev_move, true) <= bound)
            {
                return alpha;
            }
        }
        ...
        if (do_null && !is_pv_node && depth >= CONFIG_GET(m_config, m_nmr_min_depth) && remain > 2 && growth_count <= 8)
        {
            ...
            int r = CONFIG_GET(m_config, m_nmr_const) + depth / CONFIG_GET(m_config, m_nmr_depth) +
                    std::max(0, static_cast<int>(round(
                                 (static_score - beta) / 10000.0 * CONFIG_GET(m_config, m_nmr_beta_mult))));
        ...
        if (depth < CONFIG_GET(m_config, m_fut_margins).size() && !is_pv_node && std::abs(alpha) < param::checkmate &&
            std::abs(beta) < param::checkmate && !m_pos.has_jumps())
        {
            int static_score = adjusted_evaluation;
        ...
        // internal ID
        if (depth >= CONFIG_GET(m_config, m_iid_depth) && (is_pv_node || entry.m_flag == param::beta_flag) && tt_move.
            is_null())
        {
        ...
        for (int i = 0; i < moves.size(); ++i)
        {
            ...
            // lmp
            if (depth < CONFIG_GET(m_config, m_lmp_margins).size() && !is_pv_node && explored_moves >
                CONFIG_GET(m_config, m_lmp_margins)[depth] && !move.is_important()
            )
            {
            ...
            if (canFutilityPrune && explored_moves > 1 && !move.is_important()
            )
            {
                bool tactical = m_pos.has_jumps();
                if (!tactical)
...
public:
    ...
    move search(pos pos, int ts, int *new_score, const std::vector<std::string> &weights, const computer_config &config,
                bool verbose, int max_depth = param::max_depth)
    {
        if (m_threads == 1)
        {
        ...
        while (depth <= max_depth)
        {
            ...
            for (int i = 0; i < m_threads; ++i)
            {
                ...
                } else if (current_vote_score > -param::checkmate)
                {
                    if (current_vote_score > best_vote_score || (
                            current_vote_score == best_vote_score && (
                                thread_value(current_score, worse_score, depths[i]) > thread_value(
                                    scores[best_thread], worse_score, depths[best_thread]))))
                    {
                        best_thread = i;
                        best_score = current_score;
                        best_vote_score = current_vote_score;
            ...
            if (verbose)
            {
                ...
                printf("nodes %10d, depth %2d, value %7s (%5d), occ %.2lf%%", nodes, depth,
                       get_score(best_score).c_str(), best_score, m_tt.occupied() * 100.0);
